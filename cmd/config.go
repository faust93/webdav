...
func parseRules(raw []interface{}, scope *lib.Scope) []*lib.Rule {
	...
	for _, v := range raw {
		if r, ok := v.(map[interface{}]interface{}); ok {
			...
			if allowR, ok := r["allow_r"].(bool); ok {
				rule.Allow_r = allowR
			}

			if allowW, ok := r["allow_w"].(bool); ok {
				rule.Allow_w = allowW
				if allowW {
					rule.Allow_r = true
				}
...
func parseScopes(raw []interface{}) map[string]*lib.Scope {
...
func readConfig(flags *pflag.FlagSet) *lib.Config {
	...
	rawCors := viper.Get("cors")
	if cors, ok := rawCors.(map[string]interface{}); ok {
		parseCors(cors, cfg)
	}

	rawUsers := viper.Get("users")
